...
# Read in the map
file_name = r"day/25/input"
fuel_list=[]
with open(file_name, "r") as file:
    for line in file:
        fuel_list.append(list(line.strip()))

print(fuel_list)

def snafu_to_decimal(snafu):
    number = 0
    for multiplier,digit in enumerate(snafu[::-1]):
        if digit=="=":
            digit=-2
        elif digit=="-":
            digit=-1
            ...
            digit=int(digit)
        number = number + digit*(5**multiplier)
    return number

total = 0
for fuel_item in fuel_list:
    number = snafu_to_decimal(fuel_item)
    total = total + number
...
def decimal_to_snafu(decimal):
  ...
  for a in range(30):
    ...
    max_value = max_value+2*5**max_power
    if decimal<=max_value:
        break
  
  snafu = []
  for a in range(max_power,-1,-1):
      
      max_value = 0
      for power in range(a):
        max_value = max_value+2*5**power
        # print(max_value)
      
      if decimal>(5**a+max_value):
          digit = 2
      elif decimal>(max_value):
          digit = 1
      elif decimal>=(-max_value):
          digit = 0
      elif decimal>=(-5**a-max_value):
          digit = -1
      else:
          digit = -2

      decimal = decimal - digit*5**a

      snafu.append(str(digit).replace("-1","-").replace("-2","="))
